function results = psm_bsarp(y,x,W,ndraw,nomit,prior,impact)
...
[rho,rmin,rmax,detval,ldetflag,nsample,eflag,order,iter,~,c,T,inform_flag,a1,a2] = sarp_parse(prior,mik);
...
% check if the user handled the intercept term okay
    ...
    ind = find(tst == n); % we do have an intercept term
     ...
     elseif isempty(ind) % case of no intercept term
     cflag = 0;
     p = size(x,2);
     end;
...
% storage for draws
          bsave = zeros(ndraw-nomit,mik);
          psave = zeros(ndraw-nomit,1);
          sates_nnm = zeros(ndraw-nomit,1); %<----mi codigo
          sates_scm = zeros(ndraw-nomit,1); %<----mi codigo
          sates_srm = zeros(ndraw-nomit,1); %<----mi codigo
          ymean = zeros(n,1);
...
WtW = W'*W;
sige = 1;
hwait = waitbar(0,'Bayesian Spatial PSM (SAR): MCMC sampling');
...
while (iter <= ndraw); % start sampling;        
      ...
      % we use numerical integration to perform rho-draw
          ...
          eped = ed'*ed;
          epe0d = ed'*e0;
          rho = draw_rho(detval,epe0,eped,epe0d,n,mik,rho,a1,a2);
          % update z-values,  
% loop over i
    ...
    bsave(iter-nomit,1:mik) = bhat';
    psave(iter-nomit,1) = rho;
    % &&&&&&&&&&&&&&&&&&&&&
    % MI CODIGO AQUI ¬
    % Spatial Average Treatment Effect
     % PS matching nearest-neighbors:
          sates_nnm(iter-nomit,1) = psmatching_nnm(yin,x,bhat,impact);
        % Spatial Caliper Matching:
          sates_scm(iter-nomit,1) = psmatching_scm(yin,x,bhat,impact,W);
        % Spatial Radial Matching:
          sates_srm(iter-nomit,1) = psmatching_srm(yin,x,bhat,impact,W);
    % &&&&&&&&&&&&&&&&&&&&&
    ymean = ymean + y;
...
results.bdraw = bsave;
results.pdraw = psave;
results.sdraw_nnm = sates_nnm; % Average Treatment effects
results.sdraw_scm = sates_scm; % Average Treatment effects
results.sdraw_srm = sates_srm; % Average Treatment effects
results.bmean = c;
